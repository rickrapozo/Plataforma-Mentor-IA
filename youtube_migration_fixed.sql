-- Migração corrigida para integração com YouTube API v3
...
CREATE TABLE IF NOT EXISTS public.youtube_api_updates (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  category TEXT NOT NULL CHECK (category IN ('audiobook', 'podcast', 'lecture')),
...
CREATE INDEX IF NOT EXISTS idx_youtube_videos_view_count ON public.youtube_videos(view_count DESC);
CREATE INDEX IF NOT EXISTS idx_youtube_api_updates_category ON public.youtube_api_updates(category);
CREATE INDEX IF NOT EXISTS idx_youtube_api_updates_last_update ON public.youtube_api_updates(last_update DESC);
...
-- Nota: Se as políticas já existirem, você pode ignorar os erros de "already exists"
...
-- Inserir registros iniciais para controle de atualizações (apenas se não existirem)
INSERT INTO public.youtube_api_updates (category, last_update, videos_count, status) 
SELECT 'audiobook', '2025-01-01 00:00:00+00', 0, 'success'
WHERE NOT EXISTS (SELECT 1 FROM public.youtube_api_updates WHERE category = 'audiobook');

INSERT INTO public.youtube_api_updates (category, last_update, videos_count, status) 
SELECT 'podcast', '2025-01-01 00:00:00+00', 0, 'success'
WHERE NOT EXISTS (SELECT 1 FROM public.youtube_api_updates WHERE category = 'podcast');

INSERT INTO public.youtube_api_updates (category, last_update, videos_count, status) 
SELECT 'lecture', '2025-01-01 00:00:00+00', 0, 'success'
WHERE NOT EXISTS (SELECT 1 FROM public.youtube_api_updates WHERE category = 'lecture');
